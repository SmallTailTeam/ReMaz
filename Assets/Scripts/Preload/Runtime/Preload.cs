...
namespace SmallTail.Preload
{
    public static class Preload
    {
        ...
        public static void OnLoad()
        {
            ...
            foreach (Assembly assembly in assemblies)
            {
                ...
                foreach (Type preload in preloads)
                {
                    if (preload.BaseType == typeof(MonoBehaviour))
                    {
                        ...
                        string name = (preload.GetCustomAttribute(typeof(PreloadedAttribute)) as PreloadedAttribute)?.Name ?? "Unnamed";
                        instance.name = name;
            ...
            sw.Stop();
            Debug.Log($"Preloaded {count} {(count == 1 ? "script" : "scripts")} in {sw.Elapsed.TotalMilliseconds}");
        }
    }
}